...
function dropdownTitleCore(state, action) {
  return funcMap[action.type] && funcMap[action.type](state, action) || funcMapDefault(state, action)
}


export default function dropdownTitle (state=Immutable.Map(), action={}) {
  var myId = state.get('myId', '')

  switch (action.myClass) {
    case actionClasses.DROPDOWN_TITLE:
      return dropdownTitleCore(state, action)
    default:
